...
import com.github.ptube.NavDirections
...
import com.github.ptube.ui.activities.ZoomableImageActivity
import com.github.ptube.ui.fragments.ShortsPlayerFragment
import com.github.ptube.ui.base.BaseActivity
import com.github.ptube.ui.fragments.AudioPlayerFragment
import com.github.ptube.ui.fragments.PlayerFragment
import com.github.ptube.ui.views.SingleViewTouchableMotionLayout
...
object NavigationHelper {
    ...
    fun navigateShorts(context: Context, videoId: String, streamItem: StreamItem? = null) {
        val activity = ContextHelper.unwrapActivity<MainActivity>(context)
        
        // Clear standard queue to prevent mixed playback
        PlayingQueue.clear() 
        ...
        // Navigate to the Shorts tab using NavController
        // This ensures the bottom navigation bar updates correctly and we reside in the correct fragment
        activity.navController.navigate(R.id.shortsFragment, bundle)
        
        try {
            // minimize player if currently expanded (since we are moving to a main tab)
            if (activity.binding.mainMotionLayout.progress == 0f) {
                activity.binding.mainMotionLayout.transitionToEnd()
                activity.findViewById<SingleViewTouchableMotionLayout>(R.id.playerMotionLayout)
                    .transitionToEnd()
