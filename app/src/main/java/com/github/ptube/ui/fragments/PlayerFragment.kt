...
import com.github.ptube.api.JsonHelper
import com.github.ptube.api.obj.ChapterSegment
import com.github.ptube.api.obj.Segment
import com.github.ptube.api.obj.Streams
import com.github.ptube.api.obj.Subtitle
import com.github.ptube.compat.PictureInPictureCompat
import com.github.ptube.compat.PictureInPictureParamsCompat
import com.github.ptube.constants.IntentData
import com.github.ptube.databinding.FragmentPlayerBinding
import com.github.ptube.db.DatabaseHolder
import com.github.ptube.enums.PlayerCommand
import com.github.ptube.enums.PlayerEvent
import com.github.ptube.enums.SbSkipOptions
...
import com.github.ptube.extensions.serializableExtra
import com.github.ptube.extensions.toID
import com.github.ptube.extensions.togglePlayPauseState
...
import com.github.ptube.ui.dialogs.PlayOfflineDialog
import com.github.ptube.ui.dialogs.ShareDialog
import com.github.ptube.ui.extensions.animateDown
import com.github.ptube.ui.extensions.getSystemInsets
import com.github.ptube.ui.extensions.setOnBackPressed
import com.github.ptube.ui.extensions.setupSubscriptionButton
import com.github.ptube.ui.interfaces.OnlinePlayerOptions
import com.github.ptube.ui.listeners.SeekbarPreviewListener
import com.github.ptube.ui.models.ChaptersViewModel
import com.github.ptube.ui.models.CommentsViewModel
import com.github.ptube.ui.models.CommonPlayerViewModel
import com.github.ptube.ui.models.PlayerViewModel
import com.github.ptube.ui.sheets.BaseBottomSheet
import com.github.ptube.ui.sheets.CommentsSheet
import com.github.ptube.ui.sheets.StatsSheet
import com.github.ptube.util.OnlineTimeFrameReceiver
...
class PlayerFragment : Fragment(R.layout.fragment_player), OnlinePlayerOptions {
    ...
    private var isShortsMode = false
    ...
    private val playerListener = object : Player.Listener {
        override fun onIsPlayingChanged(isPlaying: Boolean) {
            binding.playImageView.setImageResource(if (isPlaying) R.drawable.ic_pause else R.drawable.ic_play)
        ...
        override fun onPlaybackStateChanged(playbackState: Int) {
            ...
            if (playbackState == Player.STATE_ENDED) {
                ...
                } else if (PlayerHelper.isAutoPlayEnabled(playlistId != null) && autoPlayCountdownEnabled && !isTransitioning) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        isShortsMode = arguments?.getBoolean(IntentData.isShortsPlayer, false) ?: false

        if (isShortsMode) {
            binding.playerScrollView.isGone = true
            binding.playerMotionLayout.setTransition(R.id.yt_transition) // Set valid transition
            binding.playerMotionLayout.transitionToState(R.id.start) // Ensure we are in Expanded state
            binding.playerMotionLayout.setTransition(R.id.yt_transition) // Set valid transition
            binding.playerMotionLayout.transitionToState(R.id.start) // Ensure we are in Expanded state
            binding.playerMotionLayout.gesturesEnabled = false
            
            binding.mainContainer.layoutParams.width = ViewGroup.LayoutParams.MATCH_PARENT
            binding.mainContainer.layoutParams.height = ViewGroup.LayoutParams.MATCH_PARENT
            ...
            binding.shortsTouchOverlay.isVisible = true
        }
        ...
        activity?.getSystemInsets()?.let { systemBars ->
            with (binding.root) {
                setPadding(
                    paddingLeft,
                    if (isShortsMode) paddingTop else paddingTop + systemBars.top,
                    paddingRight,
                    paddingBottom
                )
            }
        ...
        commonPlayerViewModel.isMiniPlayerVisible.observe(viewLifecycleOwner) { isMiniPlayerVisible ->
            ...
            onBackPressedCallback.isEnabled = isMiniPlayerVisible != true && !isShortsMode
        }
        ...
        binding.playerMotionLayout.addTransitionListener(object : TransitionAdapter() {
            ...
            override fun onTransitionCompleted(motionLayout: MotionLayout?, currentId: Int) {
                ...
                if (currentId == transitionStartId) {
                    ...
                    binding.player.useController = !isShortsMode
    ...
    private fun initializeOnClickActions() {
        ...
        binding.shortsShareButton.setOnClickListener {
            binding.relPlayerShare.performClick()
        }
        
        binding.shortsDescriptionButton.setOnClickListener {
             if (!this::streams.isInitialized) return@setOnClickListener
             val spannedDesc = androidx.core.text.HtmlCompat.fromHtml(
                 streams.description ?: "",
                 androidx.core.text.HtmlCompat.FROM_HTML_MODE_LEGACY
             )
             com.google.android.material.dialog.MaterialAlertDialogBuilder(requireContext())
                 .setTitle(streams.title)
                 .setMessage(spannedDesc)
                 .setPositiveButton(R.string.close, null)
                 .show()
        }

        binding.shortsCommentsButton.setOnClickListener {
            // set the max height to not cover the currently playing video
            updateMaxSheetHeight()
            commentsViewModel.videoIdLiveData.updateIfChanged(videoId)
            CommentsSheet()
                .apply { 
                    if (this@PlayerFragment::streams.isInitialized) {
                        arguments = bundleOf(IntentData.channelAvatar to streams.uploaderAvatar)
                    }
                }
                .show(childFragmentManager)
        }
    ...
    private fun updateMaxSheetHeight() {
        if (isShortsMode) {
            commonPlayerViewModel.maxSheetHeightPx = (binding.root.height * 0.7).toInt()
            chaptersViewModel.maxSheetHeightPx = commonPlayerViewModel.maxSheetHeightPx
            return
        }
