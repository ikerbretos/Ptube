...
class HomeFragment : Fragment(R.layout.fragment_home) {
    ...
    private fun showShorts(shorts: List<StreamItem>?) {
        android.util.Log.d("HomeFragment", "showShorts called with ${shorts?.size ?: "null"} items")
        if (shorts.isNullOrEmpty()) return

        makeVisible(binding.shortsTV, binding.shortsRV)
